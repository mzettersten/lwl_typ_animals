...
#model
m_3_1 <- lmer(average_corrected_target_looking ~ 1 + target_parent_typicality_z + age_c + (target_parent_typicality_z|sub_num) + (1|category), parent_typicality)
...
#### Exploratory model including age by parent report interaction
```{r}

#model
m_3_1_1 <- lmer(average_corrected_target_looking ~ target_parent_typicality_z*age_c+ (target_parent_typicality_z|sub_num) + (1|category), parent_typicality)

summary(m_3_1_1)

confint(m_3_1_1,method="Wald")

```

#### Exploratory analyses to see why age isn't a predictor in model 3
```{r}
...
m_3_1_2 <- lmer(target_parent_typicality_rating ~ typicality_condition_c + (typicality_condition_c|sub_num) + (1|category), parent_typicality)
...
#### Z-scored by category by participant experience ratings
...
#### Model accounting for distractor rating too
...
parent_typicality_rt <- parent_typicality_z %>%
  ...
  #group_by(sub_num,target_image, age_c, category,target_parent_typicality_z,target_parent_typicality_rating,trial_order,trial_number) %>%
  #summarize(N=n(),
       #     average_corrected_target_looking=mean(corrected_target_looking,na.rm=TRUE)) %>%
  #na.omit(target_parent_typicality_rating) %>%
  merge(trial_rt)

parent_typicality_rt <- parent_typicality_rt%>%
  filter(shift_type == "D-T")
...
#model
m_4_3 <- lmer(log(rt) ~ target_parent_typicality_z+age_c+(target_parent_typicality_z||sub_num) + (1|category), parent_typicality_rt)
